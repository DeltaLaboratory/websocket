...
func New(handler func(*Conn), config ...Config) fiber.Handler {
	...
	var upgrader = websocket.FastHTTPUpgrader{
		...
		CheckOrigin: func(ctx *fasthttp.RequestCtx) bool {
			...
			origin := utils.UnsafeString(ctx.Request.Header.Peek("Origin"))
...
// Close closes the underlying network connection with send a close message.
// if you don't want to send CloseMessage, use conn.Conn.Close() instead.
func (conn *Conn) Close() error {
	if err := conn.WriteControl(CloseMessage, nil, time.Now().Add(time.Second)); err != nil {
